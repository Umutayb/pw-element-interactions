...
7. **Detect stalled loops.** If the reviewer's `must-fix` list is identical to the previous cycle's (same finding-IDs with the same titles and same suggested fixes), the reviewer MUST include a top-level `stalled: true` flag in the return. The reviewer flag is the **primary signal** because the reviewer has full per-cycle context (suggested-fix wording, evidence pointers) that the orchestrator's set comparison can't see; the orchestrator-side ≥2-consecutive-identical-list check (per coverage-expansion §"Retry loop") is a defence against reviewer non-compliance, not a substitute. Reviewers that systematically omit `stalled: true` produce later stall detection (one extra cycle wasted) — set the flag whenever your must-fix list matches the prior cycle's, even if you suspect Stage A could fix it on one more try.
