...
* **Force click with auto-retry** — Clicks automatically retry with a native DOM event when pointer interception is detected. No configuration needed. Retries are phase-aware: a click whose input was already being dispatched when it timed out (call log reached `performing click action`) is treated as delivered rather than re-clicked, so toggles and drawer-openers can't be double-fired on slow environments.
...
* **`click(elementName, pageName, options?: StepOptions)`** — Clicks an element. Supports `{ strategy, withoutScrolling, ifPresent, force }`. On pointer interception it falls back to a dispatched DOM `'click'` event, logs a warning, and pushes a report-visible `interception-fallback` test annotation naming `PageName.elementName`; set `interceptionRetry: false` on the fixture to rethrow the original error instead. Retry is phase-aware: when the capped first attempt times out *after* its input was already being dispatched (Playwright call log reached `performing click action` with no interception), the click is treated as delivered instead of being re-clicked — a warning is logged and a `deadline-click` annotation is pushed — so non-idempotent controls (toggles, drawer openers) cannot be double-fired on slow environments; the next verification in the test is the gate that the click landed. Timeouts still inside an actionability waiting phase retry with the full timeout as before. Note: `force` dispatches a DOM `'click'` event directly (no pointer simulation, no actionability checks — NOT Playwright's `force: true`); `withoutScrolling` has alias semantics of `force` (no scroll into view). Rename pending in a future major.
