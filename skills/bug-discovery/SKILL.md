...
**Pre-empting reviewer must-fix items in adversarial passes.** Skim §"Must-fix calibration" in `reviewer-subagent-contract.md` before probing — the adversarial reviewer will: (a) attempt 2–3 probes you didn't try and flag any that land as `adversarial-missed` must-fix, (b) require ledger entries to have well-formed `expected:` / `observed:` / `evidence:` lines per the canonical schema, (c) for pass 5, require every verified boundary to have a regression test that actually locks the boundary (not a surrogate assertion). Cover the probe-category vocabulary breadth (`auth-tamper`, `input-tamper`, `state-skip`, `idor`, etc., per `references/adversarial-findings-schema.md`) on cycle 1 to avoid the "you didn't try X" finding.
